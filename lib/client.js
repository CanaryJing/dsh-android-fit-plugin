...
/* Model-select menu: bottom-centered panel on mobile.
   The upstream menu is absolute + right:0 anchored to the trigger, so on
   narrow screens it overflows the left edge (no adapter rule is active
   since the ⋯ sheet never opens). Center it horizontally just above the
   composer, capped to the viewport. */
html[data-dsh-mobile] [data-composer-card] [role="menu"] {
  position: fixed !important;
  left: 50% !important;
  right: auto !important;
  top: auto !important;
  bottom: calc(var(--dsh-composer-height, 152px) + 8px) !important;
